...
class GlassdoorPlatform(JobPlatform):
    ...
    async def search(
        ...
        task = (
            ...
            "the direct job URL, the visible description snippet on the card, "
            "company rating if shown, salary estimate if shown (verbatim text), "
            "application deadline if shown, and work-type (\"remote\" | "
            "\"hybrid\" | \"onsite\") from any badge, location text, or snippet. "
            "Return the results as a JSON array of objects with keys: "
            "id, title, company, location, url, description, rating, "
            "salary_range, deadline, work_type, remote."
        )
